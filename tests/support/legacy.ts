...
export async function legacy() {
  ...
  const path = resolve(
    process.env.BASELINE_SOURCE ?? "baseline",
    "app/api/data/route.ts",
  );
  const source = (await readFile(path, "utf8")).replace(
    /\s*await (?:seedIfEmpty|importCurrentProgramIfMissing)\(\);/g,
    "",
  );
    ...
    plugins: [
      {
        ...
        setup(b) {
          ...
          b.onLoad({ filter: /.*/, namespace: "test" }, () => ({
            contents: "export const env = globalThis.__testD1Environment;",
            loader: "js",
          }));
        },
      },
  ...
  global.__testD1Environment = { DB: binding };
  const module = {
    exports: {} as {
  ...
  delete global.__testD1Environment;
  const request = async (payload?: unknown, date = "2040-06-10") =>
    payload === undefined
      ? module.exports.GET(new Request(`http://test/api/data?date=${date}`))
      : module.exports.POST(
          new Request("http://test/api/data", {
            method: "POST",
  ...
  db.prepare(
    "INSERT INTO exercises (id,name,muscle,equipment) VALUES (?,?,?,?)",
  ).run(1, "Synthetic Press", "Chest", "Machine");
  db.prepare(
    "INSERT INTO exercises (id,name,muscle,equipment) VALUES (?,?,?,?)",
  ).run(2, "Synthetic Row", "Back", "Cable");
