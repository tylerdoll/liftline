...
test("existing UI creates a plan through its HTTP contract and survives reload", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  const request = page.waitForRequest(
    (r) =>
      ...
      r.method() === "POST" &&
      r.postDataJSON().action === "createPlan",
  );
  await dialog.getByRole("button", { name: /Save plan/ }).click();
  const payload = (await request).postDataJSON();
  expect(payload.name).toBe("Browser Contract Plan");
  expect(payload.days[0].exercises[0].exerciseId).toBe(1);
  ...
  await page
    ...
    .click();
  await expect(
    page.getByText("Browser Contract Plan", { exact: true }),
  ).toBeVisible();
});
test("load failure is visible and retry reloads data", async ({ page }) => {
  ...
  await expect(
    page.getByText("Synthetic service unavailable"),
  ).not.toBeVisible();
});
