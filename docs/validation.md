...
AWS accounts and alert recipients are not configured. Local Moto behavior does not demonstrate real IAM, Cognito JWT validation, CloudFront routing, cross-account replication/export, PITR timing, or recovery objectives. Protected environment credentials and real cloud smoke tests have not run. Keep `LIFTLINE_RELEASE_ENABLED` unset until the deployment runbook gates are complete.
